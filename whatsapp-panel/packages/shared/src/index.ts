...
export const productInput = z.object({sku:z.string().trim().min(2).max(40).regex(/^[A-Z0-9_-]+$/),name:z.string().trim().min(2).max(120),category:z.string().trim().max(80).default('General'),description:z.string().trim().max(1500).default(''),priceCents:z.number().int().nonnegative(),currency:z.string().length(3).default('PEN'),minimumStock:z.number().int().nonnegative().default(2),whatsappText:z.string().trim().max(2000).default(''),active:z.boolean().default(true)});
