# Deploy de JhelizTV — `jheliztv.xyz`
...
sudo certbot --nginx -d jheliztv.xyz -d www.jheliztv.xyz
```
...
En https://www.mercadopago.com.pe/developers/panel → tu app JHELIZTV → **Webhooks**:

- URL: `https://jheliztv.xyz/pedidos/webhooks/mercadopago/`
- Eventos: `payment`
...
- Apuntas `jheliztv.xyz` desde Hostinger hacia Fly.io por DNS.
...
fly secrets set \
  ...
  EMAIL_HOST_USER=... \
  EMAIL_HOST_PASSWORD=... \
  DEFAULT_FROM_EMAIL='JhelizTV <no-reply@jheliztv.xyz>'
fly deploy
```
...
```bash
fly certs create jheliztv.xyz
fly certs create www.jheliztv.xyz
fly certs show jheliztv.xyz
```
...
- [ ] `jheliztv.xyz` responde con HTTPS y muestra el home
- [ ] `/panel-jheliz-control/` carga y puedes entrar como superuser
...
## 5) Migración SEO desde el dominio anterior (`jhelizservicestv.xyz` → `jheliztv.xyz`)
...
2. Agregá la propiedad nueva `jheliztv.xyz` (verificación por DNS TXT).
...
- Verificalo con `curl`:
  ...
  # Esperado: HTTP/2 301
  # Location: https://jheliztv.xyz/productos/netflix/
  ```

### 5.3 Update canonical URLs
- `config/settings.py` ya tiene `SITE_URL=https://jheliztv.xyz` (vía .env).
...
2. Source: `jhelizservicestv.xyz`, Destination: `jheliztv.xyz`.
...
1. En Search Console (propiedad nueva) → Sitemaps → submit `https://jheliztv.xyz/sitemap.xml`.
